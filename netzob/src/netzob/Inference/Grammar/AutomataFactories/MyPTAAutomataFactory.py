...
@NetzobLogger
class MyPTAAutomataFactory(object):
    @staticmethod
    @typeCheck(list, list)
    def generate(abstractSessions, symbolList):
        ...
        for i,automaton in enumerate(chainedAutomatons):
            #print("Automaton {}".format(i))
            ...
            #print("  Number of initialState.transitions {}".format(len(initialState.transitions)))
            if initialState is not None and len(initialState.transitions) > 0:
                ...
                if isinstance(transition, OpenChannelTransition):
                    transition = MyPTAAutomataFactory._getNextChainedTransition(
                        transition)
                    if transition is None:
                        break
                else:
                    print("Automaton {} OpenChannelTransition Error".format(i))
                # We loop over each state to compare inputSymbol/outputSymbols with the states of the PTA automaton
                #now transition should be State0.transitions[0]
                while True:
                    ...
                    if isinstance(transition, CloseChannelTransition):
                        ...
                        '''
                        newTransition = True
                        if len(ptaStateA.transitions) > 0:
                            for transition in ptaStateA.transitions:
                                if isinstance(transition, CloseChannelTransition):
                                    newTransition = False
                                    break
                        if newTransition:
                    ...
                    #print("Current transition: {0} {1}".format(inputSymbol.name,outputSymbols[0].name))
                    ...
                    if len(ptaStateA.transitions) > 0:
                        for transition in ptaStateA.transitions:
                            if isinstance(transition, Transition) and transition.inputSymbol == inputSymbol and len(transition.outputSymbols) > 0 and len(transition.outputSymbols) == len(outputSymbols):
                                isSameOutputsymbol = True
                                for outputSymbol in transition.outputSymbols:
                                    if outputSymbol not in outputSymbols:
                                        isSameOutputsymbol = False
                                        break
                                # The transition is equivalent in the PTA
                                if isSameOutputsymbol:
                                    newTransition = False
                                    ptaStateA = ptaStateA.transitions[0].endState
                                    break
